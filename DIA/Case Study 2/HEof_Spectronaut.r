...
# Clean metadata and set row names
...
#------------------------------------------------------------------------------
# Design Matrix Loading and Processing
#------------------------------------------------------------------------------
message("Loading experimental design data...")

# Load design matrices
design1 <- tryCatch({
    data <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_n600_DIA_spt_design.tsv"), sep = "\t")
    data$sample_name <- paste0(data$sample_name, "_N")
    data$batch <- "N"
    data
}, error = function(e) {
...
design2 <- tryCatch({
    data <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_w600_DIA_spt_design.tsv"), sep = "\t")
    data$sample_name <- paste0(data$sample_name, "_W")
    data$batch <- "W"
    data
}, error = function(e) {
...
exp.mar <- tryCatch({
    ...
    stop("Error merging expression data: ", e$message)
})

# Align data matrices
message("Aligning expression and design matrices...")
...
# Analysis Setup
#------------------------------------------------------------------------------
message("Setting up analysis parameters...")
