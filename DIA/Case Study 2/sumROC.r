...
if(nrow(metrics.all) > 0){
    ...
    if(nrow(roc_data) > 0){
        ...
        if(exists("p_pr") && exists("p_roc")){
            ...
            # Combine plots side by side with their own legends
            combined_plots <- grid.arrange(
                p_pr, 
                p_roc, 
                ncol = 2, 
                nrow = 1,
                top = textGrob(paste("Performance Evaluation -", ANALYSIS_MODE), 
                              gp = gpar(fontsize = 18, fontface = "bold"))
            )
            
            # Save combined figure (PNG only)
            output_file_combined <- file.path(OUTPUT_DIR, "Combined_ROC_PR_curves.png")
            ggsave(output_file_combined, combined_plots, width = 18, height = 8, dpi = 300)
            
            message(sprintf("Combined figure saved as PNG: %s", output_file_combined))
