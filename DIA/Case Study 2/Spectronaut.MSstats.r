...
#------------------------------------------------------------------------------
# Spectronaut.MSstats_new.r - Differential Expression Analysis using MSstats package
#------------------------------------------------------------------------------
...
# - Spectronaut/{experiment}_n600_DIA/Spectronaut/report.tsv (Narrow window data)
# - Spectronaut/{experiment}_w600_DIA/Spectronaut/report.tsv (Wide window data)
# - Spectronaut/{experiment}_n600_DIA_Spectronaut_design_msstats.tsv (Narrow annotations)
# - Spectronaut/{experiment}_w600_DIA_Spectronaut_design_msstats.tsv (Wide annotations)
#
# Design file format:
...
SPECTRONAUT_DIR <- "../Spectronaut"
OUTPUT_DIR <- "Spectronaut_results/"
...
validate_input_files <- function(exp_name) {
    ...
    narrow_evidence_dir <- file.path(SPECTRONAUT_DIR, paste0(exp_name, "_n600_DIA"), "Spectronaut")
    wide_evidence_dir <- file.path(SPECTRONAUT_DIR, paste0(exp_name, "_w600_DIA"), "Spectronaut")
    
    # Look for report files (case-insensitive)
    narrow_files <- list.files(narrow_evidence_dir, pattern = "[Rr]eport\\.tsv$", full.names = TRUE)
    wide_files <- list.files(wide_evidence_dir, pattern = "[Rr]eport\\.tsv$", full.names = TRUE)
    
    if (length(narrow_files) == 0) {
        stop("Narrow evidence file not found in: ", narrow_evidence_dir)
    }
    if (length(wide_files) == 0) {
        stop("Wide evidence file not found in: ", wide_evidence_dir)
    }
    
    narrow_evidence <- narrow_files[1]  # Use first match
    wide_evidence <- wide_files[1]      # Use first match
    
    # Define annotation file paths
    narrow_annot <- file.path(SPECTRONAUT_DIR, paste0(exp_name, "_n600_DIA_spt_design_msstats.tsv"))
    wide_annot <- file.path(SPECTRONAUT_DIR, paste0(exp_name, "_w600_DIA_spt_design_msstats.tsv"))
...
#------------------------------------------------------------------------------
message("Starting Spectronaut MSstats analysis for narrow vs wide window comparison...")
...
for(i.exp in all.exp) {
    ...
    tryCatch({
        ...
        for(i.contrast in contrasts) {
            message(sprintf("\n    Processing contrast: %s", i.contrast))
            
            tryCatch({
                # Extract individual conditions
                cond1 <- str_split(i.contrast, "")[[1]][1]
                cond2 <- str_split(i.contrast, "")[[1]][2]
                ...
                if(length(missing_groups) > 0) {
                    ...
                    stop("kipping contrast: missing groups")
                }
                ...
                results_file <- file.path(OUTPUT_DIR,
                                         sprintf("MSstats_%s_%s.rds", i.exp, i.contrast))
...
#------------------------------------------------------------------------------
message("\n", "="*60)
message("Spectronaut MSstats analysis completed!")
message("="*60)
