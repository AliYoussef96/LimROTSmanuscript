...
# window Spectronaut experiments using the DEqMS package.
#
# Required Input Data Structure:
# - Spectronaut output directories:
#   * HEof_n600_DIA/Spectronaut/20231013_143752_Narrow_Report.tsv (Narrow window)
#   * HEof_w600_DIA/Spectronaut/20231013_160348_25fmol_Report.tsv (Wide window)
#   * HEof_n600_DIA_spt_dlfq.tsv (Narrow window quantification)
#   * HEof_w600_DIA_spt_dlfq.tsv (Wide window quantification)
#   * HEof_n600_DIA_spt_design.tsv (Narrow window design)
#   * HEof_w600_DIA_spt_design.tsv (Wide window design)
#   * Spectronaut/*_spt_design_msstats.tsv (MSstats annotations)
#
# Design file format:
...
SPECTRONAUT_DIR <- "../Spectronaut"  # Base directory for Spectronaut data
...
validate_input_files <- function(exp_narrow, exp_wide) {
    # Narrow window files
    peptide_file_narrow <- file.path("../Spectronaut/HEof_n600_DIA/Spectronaut/20231013_143752_Narrow_Report.tsv")
    design_file_narrow <- file.path(SPECTRONAUT_DIR, paste0(exp_narrow, "_DIA_spt_design.tsv"))
    exp_file_narrow <- file.path(SPECTRONAUT_DIR, paste0(exp_narrow, "_DIA_spt_dlfq.tsv"))
    msstats_design_narrow <- file.path(SPECTRONAUT_DIR, paste0(exp_narrow, "_DIA_spt_design_msstats.tsv"))
    
    # Wide window files
    peptide_file_wide <- file.path("../Spectronaut/HEof_w600_DIA/Spectronaut/20231013_160348_25fmol_Report.tsv")
    design_file_wide <- file.path(SPECTRONAUT_DIR, paste0(exp_wide, "_DIA_spt_design.tsv"))
    exp_file_wide <- file.path(SPECTRONAUT_DIR, paste0(exp_wide, "_DIA_spt_dlfq.tsv"))
    msstats_design_wide <- file.path(SPECTRONAUT_DIR, paste0(exp_wide, "_DIA_spt_design_msstats.tsv"))
...
for(i.exp in names(all.exp.pairs)) {
    ...
    tryCatch({
        ...
        peptide_narrow <- fread(files$narrow$peptide_file, nThread = 10)
        peptide_narrow <- data.frame(peptide_narrow, check.rows = F,
                                     check.names = F)
        ...
        peptide_wide <- fread(files$wide$peptide_file, nThread = 10)
        peptide_wide <- data.frame(peptide_wide, check.rows = F,
                                     check.names = F)
    ...
    tryCatch({
        ...
        narrow_cols <- 3:25  # Adjust based on actual column range
        wide_cols <- 3:26    # Adjust based on actual column range
    ...
    for(i.Contrasts in Contrasts) {
        ...
        tryCatch({
            ...
            if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1) {
                ...
                df.temp.imputed <- impSeq(df.temp)
                df.temp.imputed <- data.frame(df.temp.imputed, check.rows = F, check.names = F)
                
                # Peptide count analysis (following Case study 1 Spectronaut approach)
                message("      Calculating peptide counts per protein...")
                pep.count.table <- tryCatch({
                    # Step 1: Count peptides per protein per file/run (as in Case study 1 Spectronaut)
                    peptide_counts <- exp_combined %>%
                        group_by(PG.ProteinAccessions, R.FileName) %>%
                        summarise(count = n(), .groups = 'drop')
                    
                    # Handle UPS spike-ins for HEof experiments (as in Case study 1)
                    if(i.exp %in% c("HEof")) {
                        peptide_counts$PG.ProteinAccessions <- str_remove(peptide_counts$PG.ProteinAccessions, 
                                                                         fixed("_HUMAN_UPS"))
                    }
                    
                    # Filter for current contrast samples only

                    current_files <- basename(tools::file_path_sans_ext(design.temp$file))
                    peptide_counts_filtered <- peptide_counts[peptide_counts$R.FileName %in% current_files, ]
                    peptide_counts_filtered <- peptide_counts[peptide_counts$R.FileName %in% basename(tools::file_path_sans_ext(design.temp$file)),]
                    if(nrow(peptide_counts_filtered) == 0){
                    peptide_counts_filtered <- peptide_counts[peptide_counts$R.FileName %in% clean_filename(design.temp$file),]
                    }
                    if(nrow(peptide_counts_filtered) == 0){
                    stop("peptide_counts_filtered is zero")
                    }
                    ...
                    # Create protein ID mapping for df.temp (following Case study 1 Spectronaut logic)
                    if(i.exp %in% c("HEof")) {
                        df.temp.imputed$protein_ID <- sapply(row.names(df.temp.imputed), function(x) {
                            if(grepl("HUMAN_UPS|HUMAN|UPS", toupper(x))) {
                                x <- toupper(x)
                                ...
                                return(str_split_fixed(x, fixed("|"), 3)[,2])
                            }
                        })
                    } else {
                        df.temp.imputed$protein_ID <- sapply(row.names(df.temp.imputed), function(x) {
                    ...
                    not.in.peptide <- df.temp.imputed[!df.temp.imputed$protein_ID %in% min_counts$PG.ProteinAccessions,]
                    ...
                    # Match proteins between expression data and peptide counts (as in Case study 1)
                    min_counts_matched <- min_counts[min_counts$PG.ProteinAccessions %in% df.temp.imputed$protein_ID, ]
                    df.temp.matched <- df.temp.imputed[df.temp.imputed$protein_ID %in% min_counts_matched$PG.ProteinAccessions, ]
                    
                    # Remove NA rows (as in Case study 1)
                    df.temp.matched <- df.temp.matched[!is.na(df.temp.matched$protein_ID),]
                    min_counts_matched <- min_counts_matched[!is.na(min_counts_matched$PG.ProteinAccessions),]
                    
                    # Ensure matching order (as in Case study 1)
                    row.names(min_counts_matched) <- min_counts_matched$PG.ProteinAccessions
                    min_counts_matched <- min_counts_matched[df.temp.matched$protein_ID, ]
                    
                    # Validate matching (as in Case study 1)
                    if(!identical(min_counts_matched$PG.ProteinAccessions, df.temp.matched$protein_ID)) {
                        stop("Protein IDs don't match between expression and peptide count data")
                    }
                    # Set row names to match expression data (as in Case study 1)
                    ...
                    if(!identical(row.names(min_counts_matched), row.names(df.temp.matched))) {
                        stop("Protein IDs don't match between expression and peptide count data")
                    }
                    ...
                    # Update df.temp.imputed to match filtered proteins (remove protein_ID column)
                    df.temp.matched$protein_ID <- NULL
