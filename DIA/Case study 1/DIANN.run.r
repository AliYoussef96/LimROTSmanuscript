...
for(i.exp in all.exp){
  ...
  exp.mar <- read.csv(file.path("DIANN", paste0(i.exp, "_LFQ_DIANN_dlfq_pro_intensity.tsv")), sep = "\t")
  design <- read.csv(file.path("DIANN", paste0(i.exp, "_LFQ_DIANN_design.tsv")), sep = "\t")
  ...
  for(i.Contrasts in Contrasts){
    ...
    if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1){
      ...
      saveRDS(se, file.path("DIANN_results", paste0("LimROTS_", i.exp, "_", i.Contrasts, ".rds")))
      rm(se); gc()
      ...
      saveRDS(rots_results, file.path("DIANN_results", paste0("ROTS_", i.exp, "_", i.Contrasts, ".rds")))
      rm(rots_results); gc()
      ...
      saveRDS(limma.res, file.path("DIANN_results", paste0("Limma_", i.exp, "_", i.Contrasts, ".rds")))
      rm(limma.res); gc()
      ...
      saveRDS(SAM.res, file.path("DIANN_results", paste0("SAM_", i.exp, "_", i.Contrasts, ".rds")))
      rm(SAM.res); gc()
      ...
      saveRDS(ttest.res, file.path("DIANN_results", paste0("ttest_", i.exp, "_", i.Contrasts, ".rds")))
      rm(ttest.res); gc()
      ...
      saveRDS(anova.res, file.path("DIANN_results", paste0("ANOVA_", i.exp, "_", i.Contrasts, ".rds")))
      rm(anova.res); gc()
    }
  }
}
