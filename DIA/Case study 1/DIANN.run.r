...
data.file <- list.files("data/", pattern = "PXD")
...
for(i.exp in all.exp){
    ...
    exp.mar <- read.csv(paste0("../DIANN/", i.exp , "_DIA_DIANN_dlfq.tsv"), sep = "\t")
    design <- read.csv(paste0("../DIANN/", i.exp , "_DIA_DIANN_design.tsv"), sep = "\t")
...
for(i.exp in all.exp){
    ...
    exp.mar <- read.csv(paste0("../DIANN/", i.exp , "_DIA_spt_dlfq.tsv"), sep = "\t")
    design <- read.csv(paste0("../DIANN/", i.exp , "_DIA_spt_design.tsv"), sep = "\t")
    Contrasts <- combn(design$condition, 2, simplify = F)
    Contrasts <- unique(sapply(Contrasts, function(x) paste(sort(x), collapse = "")))
    Contrasts <- Contrasts[!sapply(Contrasts, function(x) length(unique(strsplit(x, "")[[1]])) == 1)]
    row.names(exp.mar) <- exp.mar$Protein
    ...
    for(i.Contrasts in Contrasts){
        ...
        if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1){
            ...
            saveRDS(limma.results, paste0("DIANN_resutls/", "Limma_" , i.exp , "_" , i.Contrasts, ".rds"))
            ...
            saveRDS(SAM.results, paste0("DIANN_resutls/", "SAM_" , i.exp , "_" , i.Contrasts, ".rds"))
            remove(SAM.results)
            gc()
            ...
            saveRDS(i.test.result, paste0("DIANN_resutls/", "ttest_" , i.exp , "_" , i.Contrasts, ".rds"))
            ...
            anova.results = data.frame(row.names(df.temp),
                                       ...
                                       P.Value=ord.p, 
                                       adj.P.Val = ord.q)
            saveRDS(anova.results, paste0("DIANN_resutls/", "ANOVA_" , i.exp , "_" , i.Contrasts, ".rds"))
