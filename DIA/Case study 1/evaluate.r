...
results.files <- list.files("spectronaut_resutls/")
...
for(i.file in results.files){
    ...
    result.read <- readRDS(paste0("spectronaut_resutls/" , i.file))
    ground_truth_name <- colnames(design)[2]
    ...
    FC.ground_truth <- readRDS(paste0("spectronaut_resutls/" , FC.ground_truth))
    ...
    saveRDS(roc_curve, paste0("roc_curve_objects_Spectronaut/" , i.file))
    }
...
write.csv(metrics.all , paste0("metrics_Spectronaut/" , "FDR_" , fdr_threshold ))
