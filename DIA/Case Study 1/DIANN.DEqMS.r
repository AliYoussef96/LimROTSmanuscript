...
#   * {experiment}_DIA_DIANN_dlfq.tsv (protein quantification data)
...
DIANN_DIR <-  "../DIANN"  # DIANN directory
...
validate_input_files <- function(exp_name) {
    peptide_file <- file.path(DIANN_DIR, paste0(exp_name  , "_DIA"), "DIANN/report.tsv")
    design_file <- file.path(DIANN_DIR, paste0(exp_name, "_DIA_DIANN_design.tsv"))
    exp_file <- file.path(DIANN_DIR, paste0(exp_name, "_DIA_DIANN_dlfq.tsv"))
    
    if (!file.exists(peptide_file)) stop("Peptide_file file not found: ", peptide_file)
    if (!file.exists(design_file)) stop("Design file not found: ", design_file)
    if (!file.exists(exp_file)) stop("Exp_file file not found: ", exp_file)
...
for(i.exp in all.exp) {
    ...
    # Format protein data
    peptide <- peptide %>% group_by(Protein.Group, Run) %>% summarise(count = n())
    peptide <- data.frame(peptide, check.rows = F, check.names = F)
    if(i.exp %in% c("HEof_w600" , "HEof_n600")){
        peptide$Protein.Group <- str_split_fixed(peptide$Protein.Group, fixed("ups"), 2)[,1]
    }
    ...
    for(i.Contrasts in Contrasts) {
        ...
        peptide.temp <- peptide[peptide$Run %in% basename(tools::file_path_sans_ext(design.temp$file)),]
        if(nrow(peptide.temp) == 0){
          peptide.temp <- peptide[peptide$Run %in% clean_filename(design.temp$file),]
        }
        if(nrow(peptide.temp) == 0){
          stop("peptide.temp is zero")
        }
        peptide.temp <- peptide.temp %>% group_by(Protein.Group) %>% 
        ...
        df.temp$protein_ID <- str_remove(df.temp$protein_ID , "UPS")
        
        not.in.peptide <- df.temp[!df.temp$protein_ID %in% peptide.temp$Protein.Group,]
        ...
        peptide.temp <- peptide.temp[peptide.temp$Protein.Group %in% df.temp$protein_ID,]
        
        df.temp <- df.temp[df.temp$protein_ID %in% peptide.temp$Protein.Group,]
        
        df.temp <- df.temp[!is.na(df.temp$protein_ID),]
        peptide.temp <- peptide.temp[!is.na(peptide.temp$Protein.Group),]
        
        row.names(peptide.temp) <- peptide.temp$Protein.Group
        peptide.temp <- peptide.temp[df.temp$protein_ID,]
        if(!identical(peptide.temp$Protein.Group , df.temp$protein_ID)){
            stop("Not identical")
        }
