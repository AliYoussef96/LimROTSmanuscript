...
#------------------------------------------------------------------------------
# Spectronaut.DEP.r - Differential Expression Analysis using DEP package
#------------------------------------------------------------------------------
...
validate_input_files <- function(exp_name) {
  intensity_file <- file.path(INPUT_DIR, paste0(exp_name, "_DIA_spt_dlfq.tsv"))
  design_file <- file.path(INPUT_DIR, paste0(exp_name, "_DIA_spt_design.tsv"))
...
all.exp <- list.files(INPUT_DIR, pattern = '_DIA_spt_dlfq')
all.exp <- unique(str_split_fixed(all.exp, "_DIA_spt_", 2)[,1])
