...
#------------------------------------------------------------------------------
# Spectronaut.MSstats.r - Differential Expression Analysis using MSstats package
#------------------------------------------------------------------------------
...
validate_input_files <- function(exp_name) {
    evidence_file <- file.path(SPECTRONAUT_DIR, paste0(exp_name , "_DIA"), "Spectronaut/")
    evidence_file <- file.path(evidence_file, list.files(evidence_file, pattern = "Report.tsv"))
    design_file <- file.path(SPECTRONAUT_DIR, paste0(paste0(exp_name , "_DIA"), "_spt_design.tsv"))
    msstats_file <- file.path(SPECTRONAUT_DIR, paste0(paste0(exp_name , "_DIA"), "_spt_design_msstats.tsv"))
...
for(i.exp in all.exp) {
    ...
    tryCatch({
        ...
        quant <- SpectronauttoMSstatsFormat(
        ...
        for(i.cnt in Contrasts) {
            tryCatch({
                ...
                test.results <- groupComparison(contrast.matrix = comparison,
                                              ...
                                              use_log_file = FALSE , numberOfCores = 15)
