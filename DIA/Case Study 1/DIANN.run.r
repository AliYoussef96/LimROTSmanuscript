...
#------------------------------------------------------------------------------
# DIANN.run.r - Multi-Method Differential Expression Analysis
#------------------------------------------------------------------------------
...
# - DIANN directory containing:
#   * {experiment}_DIA_DIANN_dlfq.tsv (intensity data)
#   * {experiment}_DIA_DIANN_design.tsv (experimental design)
#
# Design file format:
...
validate_input_files <- function(exp_name) {
    intensity_file <- file.path(INPUT_DIR, 
                               paste0(exp_name, "_DIA_DIANN_dlfq.tsv"))
    design_file <- file.path(INPUT_DIR, 
                            paste0(exp_name, "_DIA_DIANN_design.tsv"))
...
all.exp <- list.files(INPUT_DIR, pattern = 'DIA_DIANN_dlfq')
        ...
        if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1) {
            ...
            tryCatch({
                ...
                sample_info <- data.frame(
                    ...
                    group = factor(design.temp$condition)
                )
            ...
            tryCatch({
                ...
                contrast_matrix <- makeContrasts(
                    contrasts = paste0("group" ,i.Contrasts2, "-", "group", i.Contrasts1),
