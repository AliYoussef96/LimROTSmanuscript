...
results.files <- list.files("DIANN_results/")
...
for(i.file in results.files){
    ...
    result.read <- readRDS(paste0("DIANN_results/" , i.file))
    ground_truth_name <- colnames(design)[2]
    ...
    FC.ground_truth <- readRDS(paste0("DIANN_results/" , FC.ground_truth))
    ...
    saveRDS(roc_curve, paste0("roc_curve_objects_DIANN/" , i.file))
    }
...
write.csv(metrics.all , paste0("metrics_DIANN/" , "FDR_" , fdr_threshold ))
