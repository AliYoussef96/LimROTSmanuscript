...
unique(metrics.melt$DEA)

metrics.melt <- metrics.melt[metrics.melt$DEA %in% c("DEP", "Limma", "LimROTS", "ANOVA"),]

# Define custom color mapping for each DEA method using lancet palette
lancet_colors <- pal_lancet()(9)  # Get lancet palette colors
tool_colors <- c(
  "DEP" = lancet_colors[2],      
  "Limma" = lancet_colors[4],    
  "LimROTS" = lancet_colors[5],  
  "ANOVA" = lancet_colors[1]     
)
...
for(score in unique(metrics.melt$X)){
  ...
  p <- ggbetweenstats(
    ...
    scale_fill_manual(values = tool_colors) +              # Apply custom colors to fill
    scale_color_manual(values = tool_colors)               # Apply custom colors to outline
