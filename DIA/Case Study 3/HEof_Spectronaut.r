...
#------------------------------------------------------------------------------
# HEof_Spectronaut.r - Multi-Method Differential Expression Analysis for HEof Study
#------------------------------------------------------------------------------
# This script performs differential expression analysis comparing narrow and wide
# window Spectronaut proteomics data using multiple statistical methods:
# - LimROTS
# - ROTS
# - Limma
...
# - HEof_n600_DIA_Spectronaut_dlfq.tsv (Narrow window data)
# - HEof_w600_DIA_Spectronaut_dlfq.tsv (Wide window data)
# - HEof_*_DIA_Spectronaut_design.tsv (Experimental designs)
#
# Design file format:
...
# Data Loading
#------------------------------------------------------------------------------
message("Loading Spectronaut output data...")

# Load narrow window data
exp.mar1 <- tryCatch({
    data <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_n600_DIA_spt_dlfq.tsv"), sep = "\t")
...
exp.mar2 <- tryCatch({
    data <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_w600_DIA_spt_dlfq.tsv"), sep = "\t")
    data[data == 0] <- NA  # Replace 0s with NA
    data
}, error = function(e) {
...
exp.mar1[, 3:25] <- log2(exp.mar1[, 3:25] + 1)
...
# Load design files
design1 <- tryCatch({
    design <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_n600_DIA_spt_design.tsv"), sep = "\t")
...
design2 <- tryCatch({
    design <- read.csv(file.path(SPECTRONAUT_DIR, "HEof_w600_DIA_spt_design.tsv"), sep = "\t")
    if (!all(c("sample_name", "condition", "replicate") %in% colnames(design))) {
...
#------------------------------------------------------------------------------
# Contrast Generation and Analysis
#------------------------------------------------------------------------------
message("Generating and analyzing contrasts...")

# Create all pairwise contrasts
Contrasts <- tryCatch({
    # Generate all possible pairs
    pairs <- combn(design$condition, 2, simplify = FALSE)
    # Combine and sort each pair
    contrasts <- unique(sapply(pairs, function(x) paste(sort(x), collapse = "")))
    # Remove self-comparisons
    contrasts <- contrasts[!sapply(contrasts, function(x) length(unique(strsplit(x, "")[[1]])) == 1)]
    
    if (length(contrasts) == 0) {
        stop("No valid contrasts generated")
    }
    
    contrasts
}, error = function(e) {
    stop("Error generating contrasts: ", e$message)
})

# Ensure design and expression data synchronization
...
for (i.Contrasts in Contrasts) {
    message(sprintf("\nProcessing contrast: %s", i.Contrasts))
    ...
    if (select.col.Contrasts1 > 1 && select.col.Contrasts2 > 1) {
        message("Sufficient samples in both groups. Proceeding with analysis...")
        
        # Prepare data subset for current contrast
        tryCatch({
            ...
            stop(sprintf("Error processing contrast %s: %s", i.Contrasts, e$message))
        })
