...
results.files <- list.files("Spectronaut_results//")
...
for(i.file in results.files){
    ...
    # Load DEA results
    result.read <- readRDS(paste0("Spectronaut_results//" , i.file))
    ...
    if(length(unique(ground_truth)) != 2){
        ...
        if(dea.tool == "MSstats"){
            proteins <- read.csv( paste0( "../Spectronaut/" , exp.name_ , "_DIA_spt_all_proteins.tsv"), sep = "\t" )
