...
#design$new.cond <- c(rep("A" , 6), rep("B" , 6) , rep("C" , 6) , rep("D" , 6))
...
###########
#design$condition <- design$new.cond
...
for(i.Contrasts in Contrasts){
    ...
    if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1){
        ...
        ecoli <- sample(which(grepl("ECOLI", row.names(df.temp))), 200)
        ...
        limma.results = topTable(fit3, adjust="BH", coef = paste0( "group", i.Contrasts1 , "-" , "group" , i.Contrasts2)  ,sort.by = 'logFC', n=Inf)
