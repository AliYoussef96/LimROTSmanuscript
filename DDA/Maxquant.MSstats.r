...
for(i.exp in all.exp){
    ...
    exp.mar <- read.csv(paste0("../../", i.exp, "/Maxquant/TOP0/combined_protein.tsv"), sep = "\t")
    
    # Read design file
    design <- read.csv(paste0("../../Maxquant/", i.exp , "_Maxquant_design.tsv"), sep = "\t")
    
    # Read MSstats formatted evidence file
    evidence <- read.csv(paste0("../../", i.exp, "/Maxquant/TOP0/MSstats.csv"))
    
    # Read MSstats annotation
    annot <- read.csv(paste0("../../Maxquant/", i.exp , "_Maxquant_design_msstats.tsv"), sep = "\t")
    ...
    # Convert Maxquant evidence and annotation to MSstats format
    raw <- MaxquanttoMSstatsFormat(
        annotation = annot, 
        input = evidence)    
    ...
    for(i.Contrasts in Contrasts){
        ...
        # Save result table
        saveRDS(testResultOneComparison$ComparisonResult, paste0("Maxquant_results/", "MSstats_" , i.exp , "_" , i.Contrasts, ".rds"))
