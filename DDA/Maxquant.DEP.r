...
#------------------------------------------------------------------------------
# Maxquant.DEP.r - Differential Expression Analysis using DEP package
#------------------------------------------------------------------------------
...
validate_input_files <- function(exp_name) {
  intensity_file <- file.path(INPUT_DIR, paste0(exp_name, "_LFQ_Maxquant_dlfq_pro_intensity.tsv"))
    ...
    for(i.Contrasts in Contrasts) {
        ...
        if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1) {
            tryCatch({
                ...
                message("Error in DEP analysis for contrast ", i.Contrasts, ": ", e$message)
