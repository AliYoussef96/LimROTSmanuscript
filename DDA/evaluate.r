...
ANALYSIS_MODE <- "FragPipe"  # Change this to switch between MaxQuant and FragPipe
...
for(i.file in results.files){
    ...
    if(length(unique(ground_truth)) != 2){
        ...
        if(dea.tool == "MSstats"){
            proteins <- read.csv(file.path(DATA_DIR, paste0(exp.name_, "_LFQ_", ANALYSIS_MODE, "_all_proteins.tsv")), sep = "\t")
    ...
    if(pass){
        ...
        common.df.see  <- common.df[!is.na(common.df$pvalue),]
        
        common.df$method_score <- 1 - common.df$fdr
        
        if(nrow(common.df.see) != 0){
...
output_file <- file.path(OUTPUT_DIR, sprintf("metrics_%s_FDR%.2f_%s.csv", 
                                           ANALYSIS_MODE, fdr_threshold, timestamp))
