...
#------------------------------------------------------------------------------
# FragPipe.DEqMS.r - Differential Expression Analysis using DEqMS package
#------------------------------------------------------------------------------
...
#   * {experiment}/FragPipe/TOP0/combined_protein.tsv (protein quantification data)
...
validate_input_files <- function(exp_name) {
    peptide_file <- file.path(INPUT_DIR,  paste0(exp_name , "_LFQ") , "FragPipe/TOP0/combined_protein.tsv")
    design_file <- file.path(INPUT_DIR, paste0(exp_name, "_LFQ_FragPipe_design.tsv"))
    exp_file <- file.path(INPUT_DIR, paste0(exp_name, "_LFQ_FragPipe_dlfq_pro_intensity.tsv"))
...
for(i.exp in all.exp) {
    ...
    for(i.Contrasts in Contrasts) {
        ...
        tryCatch({
            ...
            peptide.razor <- which( grepl(".Total.Spectral.Count",colnames(peptide)) & 
                                        colnames(peptide) != "Combined.Total.Spectral.Count" )
            
            colnames(peptide)[peptide.razor]
            matches <- str_remove(colnames(peptide)[peptide.razor] , fixed(".Total.Spectral.Count") )
            ...
            matches <- paste0(matches , ".Total.Spectral.Count") 
