# Configuration
#------------------------------------------------------------------------------
# Set analysis mode: "MaxQuant" or "FragPipe"
ANALYSIS_MODE <- "MaxQuant"  # Change this to switch between MaxQuant and FragPipe

# Set paths based on analysis mode
RESULTS_DIR <- if(ANALYSIS_MODE == "MaxQuant") "Maxquant_results" else "FragPipe_results"
DATA_DIR <- if(ANALYSIS_MODE == "MaxQuant") "../Maxquant" else "../FragPipe"
OUTPUT_DIR <- if(ANALYSIS_MODE == "MaxQuant") "metrics_Maxquant" else "metrics_FragPipe"
...
# Ground truth function for DDA datasets
get_ground_truth_DDA <- function(dataset_name) {
  # Ground truth mapping for DDA datasets based on trueDE column
  ground_truth_map <- list(
    # Human+Yeast+E.coli datasets - trueDE: YEAST;ECOLI
    "HYE5600735" = c("YEAST", "ECOLI"),
    "HYE6600735" = c("YEAST", "ECOLI"), 
    "HYEqe735" = c("YEAST", "ECOLI"),
    "HYEtims735" = c("YEAST", "ECOLI"),
    "HYEtims777" = c("YEAST", "ECOLI"),
    ...
    # Yeast+UPS1 datasets - trueDE: UPS
    "YUltq006" = c("UPS"),
    "YUltq099" = c("UPS"), 
    "YUltq819" = c("UPS")
  )
...
for(i.file in results.files){
    ...
    ground_truth_organisms <- get_ground_truth_DDA(exp.name)
    ...
    if(length(unique(ground_truth)) != 2){
        ...
        if(dea.tool == "MSstats"){
            proteins <- read.csv(file.path(DATA_DIR, paste0(exp.name, "_LFQ_", ANALYSIS_MODE, "_all_proteins.tsv")), sep = "\t")
...
if(nrow(metrics.all) > 0){
    ...
    pr_data <- metrics.all %>%
        ...
        group_modify(~{
            ...
            if(length(fg) == 0 || length(bg) == 0) return(tibble())
    ...
    if(nrow(roc_data) > 0){
        p_roc <- ggplot(roc_data, aes(x = fpr, y = tpr, group = `Method(AUC)`, color = `Method(AUC)`)) +
          ...
          guides(color = guide_legend(title = NULL, ncol = 2, byrow = TRUE))
    ...
    stop("No valid data found for ROC/PR curve generation")
}
