...
#------------------------------------------------------------------------------
# Maxquant.run.r - Multi-Method Differential Expression Analysis
#------------------------------------------------------------------------------
...
# - MaxQuant directory containing:
#   * {experiment}_LFQ_MaxQuant_dlfq_pro_intensity.tsv (intensity data)
...
INPUT_DIR <- "Maxquant/"  # Directory containing MaxQuant output
...
validate_input_files <- function(exp_name) {
    intensity_file <- file.path(INPUT_DIR, 
                               paste0(exp_name, "_LFQ_Maxquant_dlfq_pro_intensity.tsv"))
    design_file <- file.path(INPUT_DIR, 
                            paste0(exp_name, "_LFQ_Maxquant_design.tsv"))
    ...
    for(i.Contrasts in Contrasts) {
        ...
        if(select.col.Contrasts1 > 1 & select.col.Contrasts2 > 1) {
            ...
            tryCatch({
                ...
                stop("Critical error in SAM analysis - stopping execution")
            })
