...
#------------------------------------------------------------------------------
# FragPipe.DEP.r - Differential Expression Analysis using DEP package
#------------------------------------------------------------------------------
...
#   * {experiment}_LFQ_FragPipe_design.tsv (experimental design)
#
# Design file format:
...
validate_input_files <- function(exp_name) {
  intensity_file <- file.path(INPUT_DIR, paste0(exp_name, "_LFQ_FragPipe_dlfq_pro_intensity.tsv"))
...
all.exp <- unique(str_split_fixed(all.exp, "_LFQ_FragPipe_", 2)[,1])
